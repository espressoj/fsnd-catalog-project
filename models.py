...
class Items(Base):
    ...
    # Relationships with other classes and include cascade delete where necessary
    item_categories = relationship("ItemCategories", cascade="save-update, merge, delete-orphan"
                                    , single_parent=True)
    item_inventory = relationship("Inventory", cascade="save-update, merge, delete-orphan"
                                    , single_parent=True)
    item_photo = relationship("ItemPhotos", cascade="save-update, merge, delete-orphan"
                                    , single_parent=True)
    item_tags = relationship("ItemTags", cascade="save-update, merge, delete-orphan"
                                    , single_parent=True)
    # Set the serialized data pattern for formatting JSON data
    @property
    def serialize(self):
...
class Inventory(Base):
    ...
    # Relate to Items and include cascade delete command and backref to inventory
    items = relationship("Items", backref='inv', cascade="delete-orphan", single_parent=True)
    # Set the serialized data pattern for formatting JSON data
    @property
    def serialize(self):
        """Return a serialized object for user"""
